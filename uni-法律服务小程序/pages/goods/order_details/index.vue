		...
		<view class='order-details'>
			...
			<view class="refund-msg" v-if="[4,5].includes(orderInfo.refund_type)">
				<view v-if="orderInfo._status.refund_name!=''">
					...
					<view class="refund-tip"><text class="iconfont icon-zhuyi-copy"></text>请按以上退货信息将服务服务退回
					</view>
				</view>
				<view v-else>
					<view class="refund-tip1 "><text class="iconfont icon-zhuyi-copy"></text>请联系管理员获取退货地址
					</view>
				</view>
			</view>
			...
			<view v-if="isGoodsReturn==false">
				<view class='nav'>
					<view class='navCon acea-row row-between-wrapper'>
						...
						<view :class="status.type == 2 ? 'on':''" v-if="orderInfo.shipping_type == 1">待收货
						</view>
				...
				<view class="writeOff" v-if="orderInfo.verify_code && orderInfo.paid == 1">
					...
					<view class="rules">
						...
						<view class="item">
							...
							<view class="info">
								{{orderInfo.shipping_type == 2?'可将二维码出示给店员扫描或提供数字核销码':'可将二维码出示给配送员进行核销'}}
							</view>
						</view>
					</view>
				</view>
			...
			<view class='wrapper'>
				...
				<view class='item acea-row row-between' v-if="orderInfo.remark">
					...
					<view class='conter'>{{orderInfo.remark}}
						<!-- #ifndef H5 -->
						<view v-if="orderInfo.virtual_type == 1" class='copy' @tap='copy(orderInfo.remark)'>复制
						</view>
						<!-- #endif -->
						<!-- #ifdef H5 -->
						<view v-if="orderInfo.virtual_type == 1" class='copy copy-data'
							:data-clipboard-text="orderInfo.remark">复制</view>
						<!-- #endif -->
					</view>
			...
			<view class='wrapper'>
				...
				<view class='actualPay acea-row row-right' v-else>
					...
					总代付：<text
						class='money font-color'>￥{{parseFloat(orderInfo.pay_price).toFixed(2)}}</text>
				</view>
			</view>
			...
			<view class='footer acea-row row-right row-middle'
				...
				<view class="more-box" v-if="moreBtn">
					<view class="more-btn" v-if="invoice_func && !invoiceData" @click="invoiceApply">申请开票
					</view>
				...
				<view class='bnt bg-color' v-if="status.type==0" @tap='pay_open(orderInfo.order_id)'>立即付款
				</view>
				<view
