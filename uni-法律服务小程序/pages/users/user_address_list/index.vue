		...
		<view class='address-management' :class='addressList.length < 1 && page > 1 ? "fff":""'>
			...
			<radio-group class="radio-group" @change="radioChange" v-if="addressList.length">
				<view class='item' v-for="(item,index) in addressList" :key="index">
					<view class='address' @click='goOrder(item.id)'>
						<view class='consignee'>收货人：{{item.real_name}}<text
					...
					<view class='operation acea-row row-between-wrapper'>
						...
						<view class='acea-row row-middle'>
							<view @click='editAddress(item.id)'><text class='iconfont icon-bianji'></text>编辑
							</view>
							<view @click='delAddress(index)'><text class='iconfont icon-shanchu'></text>删除
							</view>
						</view>
					</view>
				</view>
			...
			<view class='footer acea-row row-between-wrapper'>
				...
				<view class='addressBnt' @click='getWxAddress'><text
						class='iconfont icon-weixin2'></text>导入微信地址
				</view>
				<!-- #endif -->
				<!-- #ifdef H5-->
